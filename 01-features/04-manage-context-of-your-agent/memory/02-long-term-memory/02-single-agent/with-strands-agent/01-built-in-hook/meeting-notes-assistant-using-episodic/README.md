...
|-----------|----------|-------------|----------|
| Strands Agent | Meeting Notes | Meeting assistant that tracks decisions, action items, and learns from past meetings | [meeting-notes-assistant.py](./meeting-notes-assistant.py) |
...
If you are deploying this agent to AgentCore runtime, add episodic memory with the CLI:

```bash
npm install -g @aws/agentcore
agentcore create --name meetingassistant --model-provider bedrock

# Add episodic memory
agentcore add memory --name meetingmemory --strategies EPISODIC --expiry 90
agentcore deploy
```

Invoke with the same session ID across meetings to accumulate episodes:

```bash
agentcore invoke --runtime meetingassistant \
  --session-id "meeting-$(date +%Y%m%d)" \
...
```bash
python meeting-notes-assistant.py
```

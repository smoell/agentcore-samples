...
```bash
jupyter notebook 06-memory-cross-region-replication.py
```
...
```
├── 06-memory-cross-region-replication.py   # Main tutorial — run this
├── README.md
...
**`06-memory-cross-region-replication.py`** — The self-contained tutorial notebook. It deploys infrastructure, creates memory records, verifies replication, tests failover/failback, and cleans up. This is what users should follow.
...
The full cross-region replication infrastructure (Lambda consumer, secondary region memory, failover scripts) must be provisioned separately using the deployment scripts in this folder.
...
```bash
python 06-memory-cross-region-replication.py
```

...
const App = () => {
	...
	const [selectedGatewayId, setSelectedGatewayId] = useState<string | null>(
		() => localStorage.getItem("lastSelectedGatewayId"),
	);
	...
	const [authMode, setAuthMode] = useState<
		"manual" | "oauth" | "identity" | "iam"
	>(
		() =>
			(localStorage.getItem("lastAuthMode") as
				| "manual"
				| "oauth"
				| "identity"
				| "iam") || "manual",
	...
	const [pendingElicitationRequests, setPendingElicitationRequests] = useState<
		...
		>
	>([]);
	const [pendingUrlElicitationRequests, setPendingUrlElicitationRequests] =
		useState<
			Array<
				PendingUrlElicitationRequest & {
					resolve: (response: ElicitationResponse) => void;
	...
	} = useDraggableSidebar(Math.round(window.innerWidth * 0.3));
	...
	useEffect(() => {
		if (selectedGatewayId) {
			localStorage.setItem("lastSelectedGatewayId", selectedGatewayId);
		} else {
			localStorage.removeItem("lastSelectedGatewayId");
		}
	}, [selectedGatewayId]);

	const handleGatewaySelect = useCallback(
		(gatewayId: string, gatewayUrl: string) => {
			setSelectedGatewayId(gatewayId);
			setSseUrl(gatewayUrl);
		},
		[],
	);
	...
	useEffect(() => {
		localStorage.setItem("lastAuthMode", authMode);
		if (authMode === "iam") {
			setCustomHeaders((prev) =>
				prev.filter((h) => h.name.trim().toLowerCase() !== "authorization"),
			);
		} else if (authMode === "manual") {
			setCustomHeaders((prev) => {
				const hasAuth = prev.some(
			...
			setCustomHeaders((prev) =>
				prev.map((h) =>
					h.name.toLowerCase() === "authorization"
						? { ...h, value: `Bearer ${token}`, enabled: true }
						: h,
				),
			);
		},
	...
	// lgtm[js/clear-text-storage-of-sensitive-data] — dev tool persists OAuth config for convenience
	useEffect(() => {
	...
	const handleResolveUrlElicitation = (
		id: number,
		response: ElicitationResponse,
	) => {
		setPendingUrlElicitationRequests((prev) => {
			const request = prev.find((r) => r.id === id);
			if (request) {
				request.resolve(response);
	...
	const callTool = async (
		...
		try {
			...
			const isUrlElicitationError =
				errorMessage.includes("-32042") ||
				errorMessage.includes("URLElicitationRequired");

			if (isUrlElicitationError) {
				// Try to parse elicitations from the error
				// The error data may be embedded in the message or in the error object
				let elicitations: Array<{
					mode: string;
				...
				try {
					...
					if (errObj.data?.elicitations) {
						elicitations = errObj.data.elicitations;
					}
				} catch {
					// Failed to extract elicitations from error object
				}
				...
				if (elicitations.length > 0) {
					...
					const toolResult: CompatibilityCallToolResult = {
						content: [
							{
								type: "text",
								text: "URL elicitation required. Please complete the authorization in the Elicitations tab.",
							},
						],
						isError: false,
					};
	...
	const applyAccessTokenAsAuthHeader = (accessToken: string) => {
		setCustomHeaders((prev) => {
			const entry = createHeaderFromBearerToken(accessToken);
			const existingIdx = prev.findIndex(
				(h) => h.name.trim().toLowerCase() === "authorization",
			);
			if (existingIdx >= 0) {
				const copy = [...prev];
				copy[existingIdx] = { ...copy[existingIdx], ...entry, enabled: true };
				return copy;
			}
			return [...prev, entry];
		});
	...
	const AuthDebuggerWrapper = () => (
		<TabsContent value="auth">
			<AuthDebugger
				...
				onApplyAccessTokenAsHeader={applyAccessTokenAsAuthHeader}
			/>
		</TabsContent>
	);
								...
								!serverCapabilities?.tools ? (
									...
									<>
										...
										<ElicitationTab
											...
											pendingUrlRequests={pendingUrlElicitationRequests}
											onResolve={handleResolveElicitation}
											onUrlResolve={handleResolveUrlElicitation}
										/>
										<RootsTab
											roots={roots}
											setRoots={setRoots}

...
import IdentityM2MAuth from "./IdentityM2MAuth";
...
interface SidebarProps {
	...
	onGatewaySelect: (gatewayId: string, gatewayUrl: string) => void;
	...
	authMode: "manual" | "oauth" | "identity" | "iam";
	setAuthMode: (mode: "manual" | "oauth" | "identity" | "iam") => void;
	onIdentityToken: (token: string) => void;
	onOpenAuthSettings: () => void;
	onCloseAuthSettings: () => void;
						...
						{showAuthConfig && (
							<>
								<div className="space-y-1 p-3 rounded border">
									<label className="flex items-center gap-2 cursor-pointer p-1.5 rounded hover:bg-accent/50">
										<input
											type="radio"
											name="auth-mode"
									...
									<label className="flex items-center gap-2 cursor-pointer p-1.5 rounded hover:bg-accent/50">
										...
										/>
										<span className="text-sm">AgentCore Identity (M2M)</span>
									</label>
									<label className="flex items-center gap-2 cursor-pointer p-1.5 rounded hover:bg-accent/50">
										<input
											type="radio"
											name="auth-mode"
											checked={authMode === "iam"}
											onChange={() => {
												setAuthMode("iam");
												onCloseAuthSettings();
											}}
										/>
										<span className="text-sm">IAM (SigV4)</span>
									</label>
								</div>

								{authMode === "iam" && (
									<div className="p-3 rounded border">
										<h4 className="text-sm font-semibold">IAM (SigV4)</h4>
										<p className="text-xs text-muted-foreground mt-1">
											Requests are signed using the server's AWS credentials
											(from aws configure). Service: bedrock-agentcore.
										</p>
									</div>
								)}
								...
								{authMode === "oauth" && (
									<div className="space-y-3 p-3 rounded border">
										<h4 className="text-sm font-semibold">OAuth 2.0 Flow</h4>
										...
										<div className="space-y-1">
											<label className="flex items-center gap-2 cursor-pointer p-1.5 rounded hover:bg-accent/50">
												<input
													...
													readOnly
												/>
												<span className="text-sm">Pre-registered Client</span>
											</label>
											<label className="flex items-center gap-2 p-1.5 rounded opacity-40 cursor-not-allowed">
												<input type="radio" name="oauth-method" disabled />
												<span className="text-sm">
													Dynamic Client Registration (DCR)
													<span className="ml-1 text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400">
														Coming soon
													</span>
												</span>
											</label>
								...
								{authMode === "identity" && (
									<div className="p-3 rounded border">
										<IdentityM2MAuth
											config={config}
											onTokenObtained={onIdentityToken}
										/>
									</div>
								)}
			...
			<div className="p-4 border-t space-y-3">
				<div className="flex items-center justify-between">
					...
					<p className="text-xs text-muted-foreground text-center mt-2">
						...
						. All credit to its contributors.
					</p>
				</div>
			</div>
		</div>

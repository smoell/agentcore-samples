"""Demo: MCP SDK calling NASA Mars InSight API via AgentCore Gateway.
...
async def main():
    ...
    async with streamablehttp_client(
        ...
        async with ClientSession(read, write) as session:
            ...
            print("\n" + "=" * 60)
            print("tools/call — getInsightWeather")
            print("=" * 60)
            insight_tool = next(
                (
                    t.name
                    for t in tools_result.tools
                    if "insight" in t.name.lower() or "weather" in t.name.lower()
                ),
                None,
            )
            if insight_tool:
                result = await session.call_tool(
                    insight_tool, {"feedtype": "json", "ver": "1.0"}
                )
                print(
                    json.dumps(

"""Demo: invoke observability gateway tools and list them.

Lists tools exposed through the observability gateway, then calls
get_order_tool to generate CloudWatch log and trace data.

Requires GATEWAY_URL and COGNITO_STACK_NAME in environment or .env.

Usage:
    uv run python scripts/observability/invoke.py
"""
...
def main():
    ...
    if get_order:
        result = mcp.call_tool(get_order, {"orderId": "123"})
        print(json.dumps(result, indent=2))
    else:
        print("  get_order_tool not found")

    print("\n" + "=" * 60)
    print("Call update_order_tool")
    print("=" * 60)
    update_order = next(
    ...
    if update_order:
        result = mcp.call_tool(update_order, {"orderId": "123"})
        print(json.dumps(result, indent=2))
    else:
        print("  update_order_tool not found")

    print("\nDone. Check CloudWatch logs and GenAI Observability dashboard for traces.")

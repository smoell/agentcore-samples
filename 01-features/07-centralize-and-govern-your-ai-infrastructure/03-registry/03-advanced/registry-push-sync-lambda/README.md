...
> **Note:** The script creates IAM roles, a Lambda function, an EventBridge rule, and
> an AgentCore identity workload. To trigger the sync manually, update any AgentCore
> runtime — EventBridge forwards the event to the Lambda, which updates the registry record.
> The cleanup block at the bottom of the script is commented out by default to preserve
> resources for testing; uncomment it to delete all created resources.

"""
Cleanup script for discovery_and_invocation_at_runtime.py
Deletes all AWS resources created during the demo in reverse order.

Usage:
    python cleanup.py

Variables are read from the current execution namespace if available. Each
section is guarded so cleanup proceeds even if some variables are missing.
"""
...
# Safely resolve variables from the calling scope, returning None if not set.
def _safe_get(name):
    """Get a variable from the execution namespace, returning None if not set."""
    try:

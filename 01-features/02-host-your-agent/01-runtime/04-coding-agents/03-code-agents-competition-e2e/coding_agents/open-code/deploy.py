"""
Deploy OpenCode runtime to AgentCore.

Prerequisites:
  - infra.config exists (run ../infra/setup.sh)
  - agent.config exists (run ./setup.sh)
...
AGENT_NAME = local.get("AGENT_NAME", "open_code")
...
def create_execution_role() -> str:
    ...
    ecr_repo = ECR_URI.split("/")[1].split(":")[0] if "/" in ECR_URI else "coding-agents-open-code"
    ...
    if existing_id:
        try:
            ...
            control.update_agent_runtime(
                ...
                description="OpenCode coding agent with Bedrock inference",
            )
...
def main():
    ...
    print("  Config:      open-code/runtime_config.json")
    print("\n  Connect: python open-code/connect.py")
    print("=" * 60)

#!/usr/bin/env bash
# Build and push the Cursor container image to ECR, then setup AgentCore Identity.
#
# AgentCore Identity (Token Vault):
#   Instead of passing CURSOR_API_KEY as a plaintext env var on the runtime,
#   we store it encrypted in AWS Secrets Manager via AgentCore Identity.
#   At container startup, run.sh fetches the key using:
#     1. get_workload_access_token(workloadName="cursor-coding-agent")
#     2. get_resource_api_key(token, credentialProviderName="cursor-api-key")
...
#   CURSOR_API_KEY=xxx ./setup.sh         # Non-interactive
...
echo "=============================================="
echo "  Cursor — Build & Push + Identity Setup"
...
# ══════════════════════════════════════════════════════════════════════════════
# This stores the CURSOR_API_KEY encrypted in Secrets Manager (via KMS) using
...
#
# Resources created:
#   - Workload Identity: "cursor-coding-agent"
#   - API Key Credential Provider: "cursor-api-key" (encrypted in Secrets Manager)
# ══════════════════════════════════════════════════════════════════════════════
...
if [ -z "${CURSOR_API_KEY:-}" ]; then
  echo ""
  echo "  Enter your CURSOR_API_KEY:"
  read -rsp "  > " CURSOR_API_KEY
  echo ""
fi

if [ -z "$CURSOR_API_KEY" ]; then
  echo "  WARNING: No CURSOR_API_KEY provided. Skipping credential provider."
  echo "  Re-run with CURSOR_API_KEY=xxx ./setup.sh to store it later."
else
  if aws bedrock-agentcore-control get-api-key-credential-provider \
    ...
    aws bedrock-agentcore-control update-api-key-credential-provider \
      --name "$CREDENTIAL_NAME" --api-key "$CURSOR_API_KEY" --region "${INFRA_REGION}" > /dev/null
  else
...
echo "  The runtime will fetch CURSOR_API_KEY at startup via run.sh:"

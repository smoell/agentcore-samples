"""
Connect to Cursor on AgentCore Runtime via WebSocket Shell.

This gives you an interactive terminal session on the microVM.
Use it to run `cursor-agent` interactively, just like a local terminal.

Usage:
    # New session (launches cursor-agent interactive)
    python connect.py
...
async def run(args):
    ...
    async with client.open_shell(
        ...
        if args.prompt:
            ...
            print("Connected! Launching Cursor...\n")
...
def main():
    parser = argparse.ArgumentParser(description="Connect to Cursor on AgentCore via WebSocket PTY")

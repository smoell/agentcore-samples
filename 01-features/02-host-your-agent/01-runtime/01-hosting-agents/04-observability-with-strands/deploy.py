"""
Deploy the Travel Agent with AgentCore Observability to AgentCore Runtime.

This agent demonstrates built-in CloudWatch GenAI Observability using the
AWS Distro for OpenTelemetry (ADOT). When deployed to AgentCore Runtime,
spans are automatically captured and sent to CloudWatch.

Steps:
1. Create an IAM execution role with AgentCore + CloudWatch permissions
...
4. Wait for READY, create endpoint, save config to runtime_config.json

Prerequisites:
    - uv installed (https://docs.astral.sh/uv/getting-started/installation/)
    - AWS CLI configured with credentials
    - CloudWatch Transaction Search enabled (see 05-infrastructure-as-code/01-enable-transaction-search/)
...
AGENT_NAME = f"travel_obs_{int(time.time()) % 100000}"
PROTOCOL = "HTTP"
...
def create_execution_role() -> str:
    ...
    try:
        resp = iam.create_role(
            ...
            Description=f"Execution role for {AGENT_NAME} (observability demo)",
        )
...
def create_runtime(role_arn: str) -> dict:
    ...
    response = control.create_agent_runtime(
        ...
        description="Travel agent with CloudWatch GenAI Observability",
    )
...
def main():
    ...
    print("=" * 60)
    print("Deploying Travel Agent with CloudWatch Observability")
    print("=" * 60)
    ...
    print("    Open CloudWatch > GenAI Observability > Bedrock AgentCore")
    print("=" * 60)

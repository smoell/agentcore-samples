"""
Deploy the weekly report generator async agent to AgentCore Runtime.

This agent generates comprehensive weekly status reports by reading data sources,
performing analysis, generating visualizations, and compiling reports asynchronously.
...
def zip_and_upload_code():
    ...
    if os.path.isdir(demo_data_dir):
        dest_demo = os.path.join(pkg_dir, "demo_data")
        shutil.copytree(demo_data_dir, dest_demo)
        print("  Added demo_data/")
...
def create_runtime(role_arn: str) -> dict:
    ...
    response = control.create_agent_runtime(
        ...
        description="Async weekly report generator — reads data sources, analyzes, and compiles reports",
    )

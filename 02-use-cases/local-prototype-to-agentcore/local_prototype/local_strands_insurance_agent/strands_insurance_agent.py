...
def log_conversation(
    role: str, content: str, tool_calls: Optional[List] = None
) -> None:
    """Log each conversation turn with timestamp and optional tool calls"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(
        f"[{timestamp}] {role}: {content[:100]}..."
        if len(content) > 100
        else f"[{timestamp}] {role}: {content}"
    )

    if tool_calls:
        for call in tool_calls:
            logger.info(
                f"  Tool used: {call['name']} with args: {json.dumps(call['args'])}"
            )


def insurance_quote_agent(
    ...
    with insurance_client:
        try:
            ...
            return {
                "message": {
                    "content": f"I'm sorry, I encountered an error: {str(e)}. Please try again later."
                }
            }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Auto Insurance Agent using Strands and MCP"
    )
    parser.add_argument(
        "--user_input",
        type=str,
    ...
    if len(sys.argv) == 1:
        print("\nUsage Examples:\n")
        print(
            '  python strands_insurance_agent.py --user_input "What insurance options are available?"'
        )
        print(
            '  python strands_insurance_agent.py --user_input "Tell me about customer cust-001"'
        )
        print(

...
def filter_policies_by_status(
    policies: List[Dict[str, Any]], status: str = None
) -> List[Dict[str, Any]]:
...
def format_policy_response(
    policy: Dict[str, Any], include_vehicles: bool = True
) -> Dict[str, Any]:
...
def create_policy_response(
    policies: List[Dict[str, Any]], params: Dict[str, Any]
) -> Dict[str, Any]:
    """Create a formatted response for policy data"""
    include_vehicles = params.get("include_vehicles", True)

    formatted_policies = [
        format_policy_response(policy, include_vehicles) for policy in policies
    ]

    ...
    def get_insurance_quote(customer_id: str, make: str, model: str, year: int) -> str:
        ...
        try:
            ...
            products_response = requests.post(
                f"{AUTO_INSURANCE_API_URL}/insurance_products", json={}
            )
            ...
            if risk_data.get("status") == "success":
                ...
                result += (
                    f"- Credit Risk: {risk_factors.get('credit_risk', 'unknown')}\n"
                )
                result += (
                    f"- Vehicle Risk: {risk_factors.get('vehicle_risk', 'unknown')}\n"
                )
                result += (
                    f"- Overall Risk: {risk_factors.get('overall_risk', 'unknown')}\n\n"
                )
    ...
    def get_all_policies() -> str:
        ...
        try:
            ...
            if data.get("status") == "success":
                ...
                return (
                    f"Error retrieving policies: {data.get('error', 'Unknown error')}"
                )
    ...
    def get_customer_policies(customer_id: str) -> str:
        ...
        try:
            response = requests.get(
                f"{AUTO_INSURANCE_API_URL}/customer/{customer_id}/policies"
            )
            ...
            if data.get("status") == "success":
                policies = data.get("policies", [])
                result = (
                    f"Policies for Customer {customer_id} (Total: {len(policies)})\n\n"
                )

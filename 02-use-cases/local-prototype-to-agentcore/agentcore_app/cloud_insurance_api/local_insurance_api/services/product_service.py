...
import logging
from local_insurance_api.services.utils import get_product_recommendation

logger = logging.getLogger("insurance_api")


def filter_products_by_id(
    products: List[Dict[str, Any]], product_id: Optional[str or List[str]]
) -> List[Dict[str, Any]]:
...
def filter_products_by_price_range(
    products: List[Dict[str, Any]], price_range: Optional[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    ...
    if min_price is not None:
        filtered_products = [
            p for p in filtered_products if p.get("base_premium", 0) >= min_price
        ]

    if max_price is not None:
        filtered_products = [
            p for p in filtered_products if p.get("base_premium", 0) <= max_price
        ]

    return filtered_products


def filter_products_by_coverage(
    products: List[Dict[str, Any]], coverage_includes: Optional[List[str]]
) -> List[Dict[str, Any]]:
        ...
        for coverage in coverage_includes:
            ...
            if coverage == "liability" and not coverage_options.get("liability"):
                ...
                if not coverage_options.get(coverage, False):
                    include_product = False
                    break
            elif coverage == "medical_payments" and not coverage_options.get(
                "medical_payments"
            ):
                include_product = False
                break
...
def filter_products_by_discounts(
    products: List[Dict[str, Any]], discount_includes: Optional[List[str]]
) -> List[Dict[str, Any]]:
...
def sort_products(
    products: List[Dict[str, Any]], sort_by: Optional[str], sort_order: str = "asc"
) -> List[Dict[str, Any]]:
    ...
    if sort_by == "price":
        return sorted(
            products, key=lambda p: p.get("base_premium", 0), reverse=reverse_sort
        )
    elif sort_by == "name":
...
def format_product_for_response(
    ...
    rating = (
        4.0 if "basic" in product["id"] else 4.5 if "standard" in product["id"] else 4.8
    )
...
def create_product_response(
    ...
    formatted_products = [
        format_product_for_response(
            product, discounts, include_details, format_type, idx
        )

        ...
        if existing_secret:
            ...
            try:
                ...
                if "--non-interactive" in sys.argv:
                    ...
                    choice = input(
                        "Do you want to: (1) Create a new secret with these credentials, or (2) Use the existing secret directly? (1/2): "
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-connection",
        action="store_true",
        help="Test database connection after setup",
    )
    parser.add_argument(
        "--verify-only",
        help="Only verify an existing secret without creating a new one",
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use existing secret directly without creating a new one",
    )
    parser.add_argument(
        "--create-new-secret",
        action="store_true",
        help="Always create a new secret even when using an existing one as source",
    )

    ...
    try:
        ...
        if existing_sgs["SecurityGroups"]:
            ...
            try:
                ...
                sg_rules = ec2.describe_security_group_rules(
                    Filters=[
                        {"Name": "group-id", "Values": [lambda_sg_id]},
                        {"Name": "egress", "Values": ["true"]},
                    ]
                )
        ...
        for db_sg_id in db_security_group_ids:
            try:
                ...
                sg_response = ec2.describe_security_groups(GroupIds=[db_sg_id])
                ...
                if not rule_exists:
                    ...
                    print(
                        f"Added inbound rule to DB security group {db_sg_id} allowing access from Lambda security group {lambda_sg_id}"
                    )
                else:
                    print(

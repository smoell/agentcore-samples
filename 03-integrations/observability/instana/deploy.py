"""
Deploy the Travel Agent with Instana observability to AgentCore Runtime.

Usage:
    cp .env.example .env  # fill in your Instana key and OTLP endpoint
    python deploy.py
"""
...
AGENT_NAME = f"instana_obs_{int(time.time()) % 100000}"
PROTOCOL = "HTTP"
...
def create_runtime(role_arn: str) -> dict:
    ...
    response = control.create_agent_runtime(
        ...
        description="Travel agent with Instana observability",
    )
...
def main():
    if not PLATFORM_ENV_VARS.get("INSTANA_KEY"):
        print(
            "ERROR: INSTANA_KEY not set. Copy .env.example → .env and fill in your credentials."
        )
        sys.exit(1)
    if not PLATFORM_ENV_VARS.get("OTEL_EXPORTER_OTLP_ENDPOINT"):
        print(
            "ERROR: OTEL_EXPORTER_OTLP_ENDPOINT not set. Set your Instana OTLP endpoint."
        )
        sys.exit(1)
    print("=" * 60)
    print("Deploying Travel Agent with Instana Observability")
    print("=" * 60)
    ...
    print("Next: python invoke.py  |  Open Instana → Analytics → filter by service")

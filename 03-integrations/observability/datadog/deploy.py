"""
Deploy the Travel Agent with Datadog LLM Observability to AgentCore Runtime.

Usage:
    cp .env.example .env  # fill in your Datadog API key
    python deploy.py
"""
...
AGENT_NAME = f"datadog_obs_{int(time.time()) % 100000}"
PROTOCOL = "HTTP"
...
def create_runtime(role_arn: str) -> dict:
    ...
    response = control.create_agent_runtime(
        ...
        description="Travel agent with Datadog LLM Observability",
    )
...
def main():
    ...
    print("  Next: python invoke.py  |  Open Datadog LLM Observability")
    print("=" * 60)

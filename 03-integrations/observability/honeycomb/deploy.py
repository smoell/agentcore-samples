"""
Deploy the Travel Agent with Honeycomb observability to AgentCore Runtime.

Usage:
    cp .env.example .env  # fill in your Honeycomb API key
    python deploy.py
"""
...
AGENT_NAME = f"honeycomb_obs_{int(time.time()) % 100000}"
PROTOCOL = "HTTP"
...
PLATFORM_ENV_VARS = {
    ...
    "HONEYCOMB_DATASET": os.getenv("HONEYCOMB_DATASET", "llmobs"),
...
def create_runtime(role_arn: str) -> dict:
    ...
    response = control.create_agent_runtime(
        ...
        description="Travel agent with Honeycomb observability",
    )
...
def main():
    if not PLATFORM_ENV_VARS.get("HONEYCOMB_API_KEY"):
        print(
            "ERROR: HONEYCOMB_API_KEY not set. Copy .env.example → .env and fill in your credentials."
        )
        sys.exit(1)
    print("=" * 60)
    print("Deploying Travel Agent with Honeycomb Observability")
    print("=" * 60)
    ...
    print("Next: python invoke.py  |  Open Honeycomb → your dataset → Traces")

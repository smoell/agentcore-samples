"""
Travel agent for AgentCore Runtime with Honeycomb observability.

Configures a custom OTel TracerProvider with BedrockInstrumentor exporting to
Honeycomb's OTLP HTTP endpoint.
DISABLE_ADOT_OBSERVABILITY=true bypasses CloudWatch so Honeycomb receives the traces.

Required env vars (set via deploy.py → create_agent_runtime environmentVariables):
    HONEYCOMB_API_KEY   — Honeycomb API key
    HONEYCOMB_DATASET   — Dataset name (default: llmobs)
    OTEL_SERVICE_NAME   — Service name visible in Honeycomb
...
# required by Strands Agents for Honeycomb LLM observability.

...
username = "testuser"
cognito_config_name = "customer_support_agent"
...
def save_customer_support_secret(secret_value):
    ...
    try:
        secrets_client.create_secret(
            Name=cognito_config_name,
            SecretString=secret_value,
            Description="Contains the Cognito Configuration for the Customer Support Agent",
        )
...
def get_customer_support_secret():
    ...
    secrets_client = boto3.client("secretsmanager", region_name=region)
    try:
        response = secrets_client.get_secret_value(SecretId=cognito_config_name)
